...
export default function Candle(mock) {
  const ref = useRef();

  // Dimension de la chart
  const margin = { top: 20, right: 50, bottom: 20, left: 0 }; // Marges pour le SVG
  const width = 541; // Largeur totale de la chart
  const height = 120; // Hauteur totale de la chart

  // Extraction des données d'activité utilisateur
  const userActivity = mock.mock.USER_ACTIVITY[0]; // On prend la première activité de l'utilisateur
  const data = userActivity.sessions.map(session => ({
    day: session.day,
    kilogram: session.kilogram,
    calories: session.calories
  })); // Transformation des sessions en un tableau d'objets avec jour, poids et calories
  ...
  useEffect(() => {
    ...
    const rightAxis = svg.append('g')
      ...
      .call(d3.axisRight(yScaleLeft).tickValues([minWeight, midWeight, maxWeight])); // Appeler l'axe Y droit
    ...
    const tooltip = d3.select('body').append('div') // Créer un div pour le tooltip
    ...
    svg.selectAll('rect')
      .on('mouseover', (event, d) => { // Quand la souris passe sur un rectangle
